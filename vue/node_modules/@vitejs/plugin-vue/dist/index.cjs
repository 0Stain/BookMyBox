...
async function transformMain(code, filename, options, pluginContext, ssr, asCustomElement) {
  ...
  if (ssr) {
    const normalizedFilename = vite.normalizePath(path__default.relative(options.root, filename));
  ...
  if ((descriptor.script?.lang === "ts" || descriptor.scriptSetup?.lang === "ts") && !descriptor.script?.src) {
    const { code: code2, map } = await vite.transformWithEsbuild(resolvedCode, filename, { loader: "ts", sourcemap: options.sourceMap }, resolvedMap);
    resolvedCode = code2;
